...
Raphael.el.getCurrentRotation = function() {
	var t = this.transform();
	var totalrotation = 0.0;
	for(var i in t) {
		if(t[i][0] == "r") {
			totalrotation = totalrotation + t[i][1];
		}
	}
...
VectorEditor.prototype.updateTracker = function(tracker){
  if(!tracker){
	...
	//NOTE: In Raphael 2.0 translate doe NOT adjust the X & Y coordinates (or affect the BBOX)...we can get away with this for the tracker if we don't ever rely on it's bbox relative to the bbox of the original shape.
    //tracker.translate(box.x - tracker.lastx, box.y - tracker.lasty)
    //now here for the magic
    /*if(shape._ && shape._.rt){
	  console.log(shape._.rt)
      tracker.rotate(shape._.rt.deg, (box.x + box.width/2), (box.y + box.height/2))
    }*/

	//We have to first move it into place based on BBOX X & Y values.
	t = tracker.shape.transform();
	//t = [["t", tracker.shape.getBBox().x, tracker.shape.getBBox().y]].concat(t);
    tracker.transform(t)
    //console.log("Rotated " + tracker.shape.getCurrentRotation() + " degrees, currently")
	//console.log(tracker.getBBox());
	//tracker.rotate(tracker.shape.getCurrentRotation(), (box.x + box.width / 2), (box.y + box.height / 2))
...
VectorEditor.prototype.showTracker = function(shape){
  var rot_offset = -14;
  var box = shape.getBBox(true); //we need the original bbox to get correct width & height values.
