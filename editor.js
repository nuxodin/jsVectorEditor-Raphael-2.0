  ...
  if(this.mode == "select" || this.mode == "delete"){
    if(this.selectbox){
      ...
      if(this.action == "move"){
		...
		var transforms = this.selected[0].transform();
		var foundrot = false;
		for(i in transforms) {
			if(transforms[i][0] == "r") {
				transforms[i][1] = deg; //update existing rotation transformation.
				this.selected[0].transform(transforms);
				foundrot = true;
				break;
			}
		}
		if(!foundrot) {
			this.selected[0].rotate(deg); //Raphael 2.0 no longer supports absolute rotation? So we are going to fake it.
		}
        ...
        var box = this.selected[0].getBBox();
        var boxa = this.selected[0].getBBox(true);
        var nxy = this.returnRotatedPoint(x, y, box.x + box.width/2, box.y + box.height/2, -this.selected[0].getCurrentRotation());
        x = nxy[0] - 5;
        y = nxy[1] - 5;
