...
VectorEditor.prototype.move = function(shape, x, y){
  ...
  //shape.attr({x: box.x + x, y: box.y + y});
  var t = shape.transform(); // Get current transform array 
  if (t.length > 0 && t[0][0] == 't') {                  // If the first command is translate, we just merge the delta to it 
      t[0][1] += x; 
      t[0][2] += y; 
  } 
  else {                                  // otherwise we have to prepend a new translate command 
      t = [["t", x, y]].concat(t); 
  } 
  shape.transform(t); // Set the transform to the new one
  //console.log(shape.getBBox().x);
  //</end non-hack>
...
VectorEditor.prototype.resize = function(object, width, height, x, y){
  if(object.type == "rect" || object.type == "image"){
    ...
    }else if(object == this.selectbox){
	  //width is negative...
      var delta = object.attr("width") + width;
      //object.translate(x?x:object.getBBox().x)+width)
      object.translate(delta, 0);
    ...
    if(height > 0){
	  ...
	  //height is negative
	  var delta = object.attr("height") + height;
      //object.attr("y", (y?y:object.attr("y"))+height)
      //this.move(object, 0, delta);
      object.translate(0, delta);
